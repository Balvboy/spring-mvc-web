...
//	}
	...
	public T otherSessionget(Class<T> entityClass,PrimaryKey id) {
	...
	/** 根据0~n个查询条件 获取所有记录 */
	@SuppressWarnings("unchecked")
	public List<T> findAllByCriteria(Class<T> entityClass,final DetachedCriteria detachedCriteria) {
		Criteria criteria = detachedCriteria.getExecutableCriteria(sessionFactory.getCurrentSession());
		criteria.setProjection(null);
		// 限定了返回结果的样式为只是查询主体,不带关联查询的对象 
		criteria.setResultTransformer(CriteriaSpecification.DISTINCT_ROOT_ENTITY);
	...
	public void findPageByHql(Page<T> page,String hql,String totalSql) {
		...
		String result = (String)queryCount.uniqueResult();
		long total = 0;
		if(result!=null){
			total =  Long.parseLong((String) queryCount.uniqueResult()) ;     //总记录数
		}        
		Query query = sessionFactory.getCurrentSession().createQuery(hql);
        long pageNo = page.getPage();
		int pageSize = page.getRows();
